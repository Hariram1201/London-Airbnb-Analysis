...
# List of columns to ignore during one-hot encoding because they are either URLs, text descriptions,
# dates, prices, or other non-categorical features unsuitable for one-hot encoding
...
# Apply one-hot encoding to all categorical variables in the dataset except those in ignoreCols
listings = onehotEncode(listings, ignoreCols)

#List of ordinal columns to be encoded with custom mappings (preserving order/levels)
ordinalCols = ['host_response_time']
#Custom ordinal mapping for 'host_response_time' where lower values represent quicker responses
customMappings = {
    'host_response_time': {
        'within an hour': 0,
...
#Apply label encoding to ordinal columns using the specified custom mappings
listings = labelEncode(listings, ordinalCols, customMappings)

# Initialise an empty dictionary for any additional stopwords to remove during tokenisation
customStopwords = {}

#Cleans and tokenises columns with text
...
#Translate reviews comments to English, storing results in new 'comments_translated' column
reviews['comments_translated'] = reviews['comments'].apply(translateToEnglish)

#Cleans and tokenises the review comments, defines a set of custom set of stopwords to exclude from tokenisation
...
for place, coords in keyLocations.items():
    ...
    listings[f'distance_to_{cleanDist}'] = listings.apply(
        lambda row: calculateDistance(row, locationCoords=coords),
        axis=1)
    ...
    listings[f'distance_to_{cleanDist}_colour'] = listings.apply(
        lambda row: getDistColours(row, distVar=f'distance_to_{cleanDist}', distanceColors=distanceClassify),
        axis=1)
